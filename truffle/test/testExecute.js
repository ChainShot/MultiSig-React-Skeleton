...
contract('MultiSig', function(accounts) {
    ...
    describe('Execute Transaction Tests', function() {
        ...
        it('should log the deposit', async function() {
            let watcher = contract.Deposit();
            const ether = 1;
            let receipt = await contract.sendTransaction({from: accounts[1], value: web3.toWei(ether, 'ether')});
            let events = await watcher.get();
            assert.equal(events[0].event, 'Deposit');
        })
        ...
        it('should only log deposits than 0', async function() {
            let watcher = contract.Deposit();
            const ether = 0;
            let receipt = await contract.sendTransaction({from: accounts[1], value: web3.toWei(ether, 'ether')});
            let events = await watcher.get();
